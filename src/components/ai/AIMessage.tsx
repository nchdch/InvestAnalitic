...
const Spark = (props: React.SVGProps<SVGSVGElement>) => (
  <svg {...props} viewBox="0 0 24 24" fill="none">
    ...
    <path d="M19 4.5l.6 1.9L21.5 7l-1.9.6L19 9.5l-.6-1.9L16.5 7l1.9-.6L19 4.5z" fill="currentColor" opacity=".85" />
  </svg>
)
...
export function AIMessage({
  ...
  return (
    <div className={['ia-msg', `ia-msg--${role}`, className].filter(Boolean).join(' ')} {...rest}>
      <span className={['ia-msg__avatar', isAI ? 'ia-msg__avatar--ai' : 'ia-msg__avatar--user'].join(' ')}>
        {isAI ? <Spark /> : (name ? name[0].toUpperCase() : 'В')}
      </span>
      <div className="ia-msg__body">
        <span className="ia-msg__name">{isAI && <Spark />}{displayName}</span>
        {typing ? (
          <div className="ia-msg__bubble ia-msg__typing" aria-label="ИИ печатает">
            <span /><span /><span />
          </div>
        ) : (
          <div className="ia-msg__bubble">{children}</div>
        )}
        {actions && !typing && <div className="ia-msg__actions">{actions}</div>}
      </div>
    </div>
  )
}
